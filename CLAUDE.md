...
Pairing is the only way a room comes to exist. `POST /api/seeks` either claims a compatible seek and writes a room with both seats filled (`createPairedRoom`), or stands a seek on the board; there is no room with a free seat and no route that creates one. The seek is REST because it happens before a socket exists; everything after that is WS. The end of a game is a `stateUpdated` carrying `status: 'finished'`, so the client has one state path instead of two.
...
Seat and presence are separate concepts. A socket close notifies the opponent of `disconnected` but leaves seats intact, so a reconnect is another `joinGame`. `joinGame` only re-attaches: a player holding neither seat is refused. Deleting an account is the one thing that empties a seat.
