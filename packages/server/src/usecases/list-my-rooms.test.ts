...
describe('listMyRooms', () => {
  ...
  it('reports the seat the player holds', async () => {
    const { rooms: store } = createTestStores(PLAYERS);
    await store.create(createRoom('white-room', ident('alice'), NOW));
    await store.create(seatPlayer(createRoom('black-room', ident('bob'), NOW), ident('alice')));
  ...
  it('does not expose the opponent playerId', async () => {
    const { rooms: store } = createTestStores(PLAYERS);
    await store.create(seatPlayer(createRoom('r1', ident('alice'), NOW), ident('bob-secret')));
    const mine = await listMyRooms(ident('alice'), store);
  ...
  it('lists a room per seat held, several at once', async () => {
    const { rooms: store } = createTestStores(PLAYERS);
    await store.create(createRoom('r1', ident('alice'), NOW));
    await store.create(createRoom('r2', ident('alice'), NOW));
